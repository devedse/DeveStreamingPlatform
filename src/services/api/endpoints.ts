...
// Build endpoint paths for a given app (defaults to main app)
export const endpoints = {
  // Get list of all streams
  getStreams: (app?: string) =>
    `/v1/vhosts/${config.ome.vhost}/apps/${app || config.ome.app}/streams`,

  // Get statistics for a specific stream
  getStreamStats: (streamName: string, app?: string) =>
    `/v1/stats/current/vhosts/${config.ome.vhost}/apps/${app || config.ome.app}/streams/${streamName}`,

  // Get detailed stream information including variants
  getStreamDetails: (streamName: string, app?: string) =>
    `/v1/vhosts/${config.ome.vhost}/apps/${app || config.ome.app}/streams/${streamName}`,
  ...
  // Pull stream endpoint (for a given app)
  createPullStream: (app?: string) =>
    `/v1/vhosts/${config.ome.vhost}/apps/${app || config.ome.app}/streams`,

  // Delete stream endpoint (for a given app)
  deleteStream: (streamName: string, app?: string) =>
    `/v1/vhosts/${config.ome.vhost}/apps/${app || config.ome.app}/streams/${streamName}`,

  // MultiplexChannel endpoints (for public/private stream management)
  getMultiplexChannels: (app?: string) =>
    `/v1/vhosts/${config.ome.vhost}/apps/${app || config.ome.app}/multiplexChannels`,

  createMultiplexChannel: (app?: string) =>
    `/v1/vhosts/${config.ome.vhost}/apps/${app || config.ome.app}/multiplexChannels`,

  deleteMultiplexChannel: (channelName: string, app?: string) =>
    `/v1/vhosts/${config.ome.vhost}/apps/${app || config.ome.app}/multiplexChannels/${channelName}`,
}
...
export interface PlaybackOptions {
  /** OME application name (defaults to main app) */
  app?: string
  /** Auth token to append to playback URLs (omit for public streams) */
  authToken?: string | null
}

// Generate playback URLs for OvenPlayer (multiple sources for protocol selection)
export const generatePlaybackSources = (streamName: string, options?: PlaybackOptions) => {
  const app = options?.app || config.ome.app
  ...
  const webrtcBaseUrl = `${config.ome.publishers.webrtcUrl}/${app}/${streamName}/multistream_webrtc`
  const llhlsBaseUrl = `${config.ome.publishers.llhlsUrl}/${app}/${streamName}/multistream_llhls.m3u8`

  // Add auth token to URLs if provided (for private stream playback)
  const webrtcUrl = token ? `${webrtcBaseUrl}?auth=${token}` : webrtcBaseUrl
  const llhlsUrl = token ? `${llhlsBaseUrl}?auth=${token}` : llhlsBaseUrl
...
export interface ThumbnailOptions {
  /** OME application name (defaults to main app) */
  app?: string
  /** Auth token for private stream thumbnails */
  authToken?: string | null
  /** Use the public thumbnail proxy (no auth required) */
...
export const generateThumbnailUrl = (streamName: string, options?: ThumbnailOptions) => {
  const app = options?.app || config.ome.app
  const proxyPath = options?.usePublicProxy ? config.api.publicThumbnailUrl : config.api.thumbnailUrl
  const timestamp = Date.now()
  const base = `${proxyPath}/${app}/${streamName}/thumb.png`
  const token = options?.authToken
